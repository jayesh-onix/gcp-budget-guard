...
│   ├── test_budget_monitor.py     # Orchestrator integration tests (18 tests)
...
│   ├── test_notification.py       # Notification + Pub/Sub tests (18 tests)
...
The test suite contains **142 tests** across 13 test files, all running with mocks (no GCP credentials needed):
...
- `test_budget_monitor.py` – 18 tests (orchestrator integration: under-budget, exceeded, warning at 80%, data quality warnings, baseline subtraction, reset with baseline + alerts + audit, partial failures; **live query first, cached fallback, zero-cost fallback**)
...
- `test_notification.py` – 18 tests (alert counter, max-2-per-service, critical-only-after-disable, reset, independent per-service counters, **multi-recipient email delivery** (sendmail recipients, To header, single-recipient fallback), Pub/Sub integration, Pub/Sub payload validation, alert via Pub/Sub without SMTP)
